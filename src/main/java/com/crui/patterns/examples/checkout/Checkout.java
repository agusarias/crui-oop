 ...
 * código dado? 
 * Se pueden identificar los siguientes patrones de diseño en el codigo:
 * 
 * - Strategy: El patrón Strategy se utiliza en la selección del medio de pago (MedioDePago). Permite cambiar la forma de pago de la orden sin modificar la logica.
 *
 * - Observer: El patrón Observer se utiliza para notificar a los listeners sobre eventos de la orden (pago realizado). Se puede ver en los Listeners.
 *
 * - Qué patrón de diseño podrías agregar para mejorar el código?
 * 
 * - Factory: Centraliza la creación de objetos de pago en PaymentFactory.

              Hace más limpio el main → no necesita if ni switch para saber cómo crear cada pago.

              Escalable → si mañana agregás otro medio de pago sin tocar el resto del sistema.

              Encapsula dependencias externas → el MercadoPagoAPI (que es una API simulada externa) queda envuelto dentro de la fábrica, y el resto del sistema no tiene que saber cómo funciona.
 *
